    ...
    <div className="max-w-7xl mx-auto pb-12 flex flex-col h-full space-y-6 relative">
      ...
      <div className="flex gap-6 flex-1 h-[calc(100vh-14rem)]">
        ...
        <div className={`flex-1 space-y-6 transition-all h-full overflow-y-auto pr-2 ${selectedEvidence ? 'w-2/3' : 'w-full'}`}>
          {/* Filters */}
        <div className="bg-white p-4 rounded-xl shadow-sm border border-accent flex flex-wrap gap-4 items-center text-sm">
          <span className="font-semibold text-text-secondary mr-2">Filters:</span>
          <select className="border border-accent rounded p-2 bg-background text-foreground">
          ...
          <input type="date" className="border border-accent rounded p-2 bg-background text-foreground" />
        </div>
        ...
        <div className="grid grid-cols-1 space-y-4">
          {mockEvidence.map((ev) => (
            <div 
              ...
              className={`bg-white p-5 rounded-xl shadow-sm border cursor-pointer transition-colors ${selectedEvidence?.id === ev.id ? 'border-primary ring-1 ring-primary' : 'border-accent hover:border-primary'}`}
            >
              <div className="flex justify-between items-start mb-2">
                <div className="flex gap-2 items-center">
                  <span className="text-xs font-semibold bg-accent text-primary px-2 py-1 rounded">{ev.sourceType}</span>
                  <span className="text-xs text-text-secondary">{ev.date}</span>
                </div>
                <span className="text-xs font-medium text-text-secondary border border-accent px-2 py-1 rounded">{ev.accessLevel}</span>
              </div>
      ...
      {selectedEvidence && (
        <div className="w-1/3 bg-white rounded-xl shadow-lg border border-primary sticky top-8 h-[calc(100vh-4rem)] overflow-y-auto animate-in slide-in-from-right-8 duration-300">
          <div className="p-4 border-b border-accent flex justify-between items-center sticky top-0 bg-white">
            <h2 className="font-bold text-primary">Evidence Viewer</h2>
          ...
          <div className="p-6 space-y-6">
            <div>
              <span className="text-xs font-semibold bg-accent text-primary px-2 py-1 rounded mb-3 inline-block">{selectedEvidence.sourceType}</span>
            ...
            <div>
              ...
              <p className="text-sm text-foreground bg-background p-3 rounded border border-accent">{selectedEvidence.provenance}</p>
            </div>

            <div>
              <h4 className="text-sm font-semibold text-text-secondary uppercase tracking-wider mb-2">Relevant Excerpt</h4>
              <p className="text-sm text-foreground leading-relaxed italic border-l-4 border-primary pl-4">{selectedEvidence.excerpt}</p>
            </div>
            ...
            <div className="pt-4 border-t border-accent">
              ...
              <ul className="space-y-2 text-sm">
                <li><a href="#" className="text-primary hover:underline">📄 Related Research: Impact Analysis 2023</a></li>
                <li><a href="/gis" className="text-primary hover:underline">🗺️ GIS Layer: Mulshi LULC Change</a></li>
                <li><a href="#" className="text-primary hover:underline">📊 Dataset: Crop Yield Stats</a></li>
              </ul>
            </div>
